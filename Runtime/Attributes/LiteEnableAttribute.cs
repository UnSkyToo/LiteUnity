...
namespace LiteUnity.Runtime.Attributes
{
    ...
    public class LiteEnableSourceAttribute : Attribute
    {
    ...
    /// <summary>
    /// 控制字段是否在编辑器上展示。
    /// 其中FieldName和FiledValue决定了依据哪个字段进行控制，该字段需要被LiteEnableSourceAttribute标记。
    /// CompareEqual字段决定FieldValue是比较相等还是不相等，默认为相等。
    /// </summary>
    [Conditional("UNITY_EDITOR")]
    [AttributeUsage(AttributeTargets.Property | AttributeTargets.Field, Inherited = true, AllowMultiple = true)]
    public class LiteEnableCheckerAttribute : Attribute
    {
        ...
        public bool Check(Dictionary<string, object> data)
        {
            ...
            {
                var isEqual = LiteTypeSystem.Equal(FieldValue, data[FieldName]);
        ...
        public static bool Check(FieldInfo field, Dictionary<string, object> sourceData)
        {
            var checkerAttr = field.GetCustomAttributes<LiteEnableCheckerAttribute>();
            return Check(checkerAttr, sourceData);
        }

        public static bool Check(PropertyInfo property, Dictionary<string, object> sourceData)
        {
            var checkerAttr = property.GetCustomAttributes<LiteEnableCheckerAttribute>();
            return Check(checkerAttr, sourceData);
        }
    }
}