...
{
    internal class LiteReorderableListWrap
    {
        ...
        {
            IsList_ = LiteTypeSystem.IsListType(data.GetType());
            ElementType_ = LiteTypeSystem.GetElementType(data.GetType());
        ...
        private bool DrawListHeader(GUIContent title)
        {
            ...
            while (diff > 0)
            {
                AddToList(LiteTypeSystem.CreateInstance(ElementType_));
                diff--;
            }
        ...
        {
            var element = LiteTypeSystem.CreateInstance(ElementType_);
            AddToList(element);
        }
